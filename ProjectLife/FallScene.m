...
@interface FallScene ()<SKPhysicsContactDelegate> {SKSpriteNode *_plant;}
...
- (id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        _background = [SKSpriteNode spriteNodeWithImageNamed:@"FALL"];
...
{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"fall" ofType:@"wav"];
    NSError *error = nil;
...
- (void) setupPlant {
    ...
    _plant =[SKSpriteNode spriteNodeWithImageNamed:@"adult.png"];
...
- (void)addNutrient {
    ...
    if(val < 0.25){
        ...
        nutrient = [SKSpriteNode spriteNodeWithImageNamed:@"acorn.png"];
        nutrient.name = @"acorn";
    }else {
        nutrient = [SKSpriteNode spriteNodeWithImageNamed:@"trash.png"];
        nutrient.name = @"trash";
    }
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    if (contact.bodyA.categoryBitMask < contact.bodyB.categoryBitMask){
        if(![contact.bodyA.node.name isEqualToString:@"acorn"] && ![contact.bodyA.node.name isEqualToString:@"trash"]){
        ...
        if(![contact.bodyB.node.name isEqualToString:@"acorn"] && ![contact.bodyB.node.name isEqualToString:@"trash"]){
    ...
    //Plant animations
    if(grow) {
        SKAction *smile = [SKAction animateWithTextures:@[[SKTexture textureWithImageNamed:@"adult.png"],
                                                          [SKTexture textureWithImageNamed:@"adult-smile1.png"],
                                                          [SKTexture textureWithImageNamed:@"adult-smile2.png"]]timePerFrame:0.033];
        SKAction *smileanim = [SKAction sequence:@[smile, [SKAction waitForDuration:1.0], [smile reversedAction]]];
        [_plant runAction:smileanim];
    } else {
        SKAction *frown = [SKAction animateWithTextures:@[[SKTexture textureWithImageNamed:@"adult.png"],
                                                          [SKTexture textureWithImageNamed:@"adult-frown1.png"],
                                                          [SKTexture textureWithImageNamed:@"adult-frown2.png"]]timePerFrame:0.033];
...
-(void) nextLevel {
    if(!self.won){
        ...
        //Transition to Winter level
        WinterScene *winterScene = [[WinterScene alloc] initWithSize:self.size];
        SKTransition *transition = [SKTransition pushWithDirection:SKTransitionDirectionDown duration:1.0];
        [self.view presentScene:winterScene transition:transition];
    }
...
-(void)save {
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setInteger:3 forKey:@"seasonFlag"]; 
    ...
    [self enumerateChildNodesWithName:@"acorn" usingBlock:^(SKNode *node, BOOL *stop) {
        if (node.position.y > 0)
        {
            [arrayOfNutrients addObject:node];
        }
