...
#define SMILE @[ \
[SKTexture textureWithImageNamed:@"teen-smile1.png"], \
[SKTexture textureWithImageNamed:@"teen-smile2.png"], \
]

@interface SummerScene ()<SKPhysicsContactDelegate> {SKSpriteNode *_plant;}
...
- (id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        _background = [SKSpriteNode spriteNodeWithImageNamed:@"SUMMER"];
        ...
        if (seasonNumber == 2) { //change for each season
            [self load];
        }
    }
...
{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"summer" ofType:@"wav"];
    NSError *error = nil;
...
- (void) setupPlant {
    ...
    _plant =[SKSpriteNode spriteNodeWithImageNamed:@"teen.png"];
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    //Nutrient disappears as it is absorbed by plant, obstacles do not
    if (contact.bodyA.categoryBitMask < contact.bodyB.categoryBitMask){
        if(![contact.bodyA.node.name isEqualToString:@"bug"]){
        ...
        if(![contact.bodyB.node.name isEqualToString:@"bug"]){
    ...
    //Plant animations
    if(grow) {
        SKAction *smile = [SKAction animateWithTextures:@[[SKTexture textureWithImageNamed:@"teen.png"],
                                                          [SKTexture textureWithImageNamed:@"teen-smile1.png"],
                                                          [SKTexture textureWithImageNamed:@"teen-smile2.png"]]timePerFrame:0.033];
        SKAction *smileanim = [SKAction sequence:@[smile, [SKAction waitForDuration:1.0], [smile reversedAction]]];
        [_plant runAction:smileanim];
    } else {
        SKAction *frown = [SKAction animateWithTextures:@[[SKTexture textureWithImageNamed:@"teen.png"],
                                                          [SKTexture textureWithImageNamed:@"teen-frown1.png"],
                                                          [SKTexture textureWithImageNamed:@"teen-frown2.png"]]timePerFrame:0.033];
...
-(void) nextLevel {
    if(!self.won){
        ...
        //Transition to Fall level
        FallScene *fallScene = [[FallScene alloc] initWithSize:self.size];
        SKTransition *transition = [SKTransition pushWithDirection:SKTransitionDirectionDown duration:1.0];
        [self.view presentScene:fallScene transition:transition];
    }
...
-(void)save {
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setInteger:2 forKey:@"seasonFlag"]; //change for each Season
